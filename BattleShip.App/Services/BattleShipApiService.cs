...
public class BattleShipApiService : IDisposable
{
    private readonly GrpcChannel _channel;
    private readonly BattleShipService.BattleShipServiceClient _client;
    private readonly GameState _gameState;

    public BattleShipApiService(GameState gameState)
    {
    ...
    public async Task<bool> StartNewGameAsync(string difficulty = "Medium")
    {
        try
        {
            ...
            {
                Letter = string.IsNullOrEmpty(s.Letter) ? ' ' : s.Letter[0],
                Size = s.Size
            }).ToList();
            ...
            // Convertir le repeated field en tableau 2D
    ...
    public async Task<bool> AttackAsync(int row, int col)
    {
        ...
        try
        {
            ...
            _gameState.Winner = response.Winner;
            ...
            _gameState.MoveHistory.Add(new MoveInfo
            {
                ...
                IsPlayerMove = true,
                IsHit = isHit,
                HitShipLetter = string.IsNullOrEmpty(response.HitShipLetter) ? null : response.HitShipLetter[0]
            });
            ...
            if (response.AiAttack != null)
            {
                _gameState.MoveHistory.Add(new MoveInfo
                {
                    ...
                    HitShipLetter = string.IsNullOrEmpty(response.AiAttack.HitShipLetter) ? null : response.AiAttack.HitShipLetter[0]
                });
            }
